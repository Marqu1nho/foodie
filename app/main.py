...
def compute(m: str) -> list[tuple[str, float]]:
    ...
    if not S.in_play:
        return []
    restrict: set[str] | None = svc.protein_sources() if S.protein_only else None
    dirs = build_directions(svc, m, S.cuisine, S.push, S.flavor)
    if dirs:
        return svc.pairings_directed(m, S.in_play, dirs, S.k, restrict_to=restrict)
    return svc.pairings(m, S.in_play, S.k, restrict_to=restrict)
...
def index() -> None:
    ...
    def set_protein_only(v: bool) -> None:
        S.protein_only = bool(v)
        canvas_view.refresh()
        top_connections.refresh()
    ...
    with ui.element("div").style(
        ...
        with ui.element("div").style(
            ...
            with ui.element("div").style(
                ...
                # protein-only toggle card
                build_protein_toggle(lambda: S.protein_only, set_protein_only)
