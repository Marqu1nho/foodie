...
def test_pairings_directed_compose_changes(epicure_service) -> None:
    ...
    single_cuisine = epicure_service.pairings_directed(
        "core", names, [(cuisine, 40.0)], k
    )
...
def test_pairings_directed_empty_and_missing(epicure_service) -> None:
    ...
    assert (
        epicure_service.pairings_directed(
            "core", [], [("cuisine:NotARealPole", 30.0)], k
        )
        == []
    )
...
# ---------------------------------------------------------------------------
# PROTEIN SOURCES / RESTRICT_TO
# ---------------------------------------------------------------------------

# Protein seeds that should be included (we check at least 4 of these exist
# in vocab AND appear in the protein set).
_PROTEIN_SEEDS = [
    "tofu",
    "chickpea",
    "lentil",
...
# Items that should NOT be in the protein set.
_NON_PROTEIN_SEEDS = ["sugar", "honey", "vanilla", "mint"]


def test_protein_sources_set(epicure_service) -> None:
    """protein_sources() returns a non-empty set with expected inclusions/exclusions."""
    ps = epicure_service.protein_sources()
    assert isinstance(ps, set)
    assert len(ps) > 0

    vocab = epicure_service.vocab()

    # At least 4 of the protein seed list must be present in the set.
    in_vocab_and_set = [n for n in _PROTEIN_SEEDS if n in vocab and n in ps]
    assert len(in_vocab_and_set) >= 4, (
        f"Expected at least 4 protein seeds in protein_sources, got {in_vocab_and_set}"
    )

    # None of the non-protein seeds should appear.
    leaked = [n for n in _NON_PROTEIN_SEEDS if n in vocab and n in ps]
    assert leaked == [], f"Non-protein items leaked into protein_sources: {leaked}"


def test_pairings_restrict_to(epicure_service) -> None:
    """pairings with restrict_to returns only items from that set; input not returned."""
    proteins = epicure_service.protein_sources()
    assert proteins, "protein_sources() must be non-empty for this test"

    results = epicure_service.pairings("core", ["rice"], 10, restrict_to=proteins)
    assert isinstance(results, list)
    for name, score in results:
        assert name in proteins, f"'{name}' is not in restrict_to set"
    returned_names = {n for n, _ in results}
    assert "rice" not in returned_names, "'rice' (input) must be excluded from results"


def test_pairings_directed_restrict_to(epicure_service) -> None:
    """pairings_directed with restrict_to returns only items from that set."""
    proteins = epicure_service.protein_sources()
    assert proteins, "protein_sources() must be non-empty for this test"

    cuisine = epicure_service.cuisines("core")[0]
    results = epicure_service.pairings_directed(
        "core", ["rice"], [(cuisine, 30.0)], 10, restrict_to=proteins
    )
    assert isinstance(results, list)
    for name, score in results:
        assert name in proteins, f"'{name}' is not in restrict_to set"
...
def test_restrict_to_none_unchanged(epicure_service) -> None:
    """pairings with restrict_to=None is identical to pairings without restrict_to."""
    names = ["apple", "cinnamon"]
    k = 8
    base = epicure_service.pairings("core", names, k)
    with_none = epicure_service.pairings("core", names, k, restrict_to=None)
    assert base == with_none, (
        "pairings(..., restrict_to=None) must match pairings(...) exactly"
    )
